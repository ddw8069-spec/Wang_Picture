package com.yupi.yupicture.infrastructure.scheduler;
...
import com.yupi.yupicture.infrastructure.config.CleanupProperties;
...
public class PictureCleanupTask {
    ...
    public void cleanOrphanPictures() {
        ...
        try {
            ...
            int totalCleaned = 0;
            for (int i = 0; i < 100; i++) {
                List<Picture> batch = pictureRepository.getBaseMapper().selectDeletedBefore(
                        threshold, cleanupProperties.getBatchSize());
                ...
                for (Picture pic : batch) {
                    try {
                        ...
                        pictureRepository.physicalDeleteById(pic.getId());
                        totalCleaned++;
                    } catch (Exception e) {
    ...
    public void cleanRejectedPictures() {
        ...
        try {
            ...
            int totalCleaned = 0;
            for (int i = 0; i < 100; i++) {
                List<Picture> batch = pictureRepository.getBaseMapper().selectRejectedBefore(
    ...
    public void archiveColdPictures() {
        ...
        try {
            ...
            int totalArchived = 0;
            for (int i = 0; i < 100; i++) {
                List<Picture> batch = pictureRepository.getBaseMapper().selectUnmodifiedBefore(
                        threshold, cleanupProperties.getBatchSize());
                if (CollUtil.isEmpty(batch)) {
                    break;
                }
